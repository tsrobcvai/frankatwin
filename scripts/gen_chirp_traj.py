...
Run:
  python scripts/gen_chirp_traj.py --base-sidecar ref.json --band low  [--out-csv …] [--out-sidecar …]
  python scripts/gen_chirp_traj.py --base-sidecar ref.json --band high [--out-csv …] [--out-sidecar …]

``--band`` picks the sweep (``CHIRP_BANDS``): ``low`` 0.1 -> 0.7 Hz at constant
amplitude, ``high`` 0.7 -> 3 Hz with the amplitude tapered as (0.7/f)^0.5
and a 1 s fade-out (sized for kp 200/20). Both are 8 s; ``low`` is the fit
run, ``high`` an optional held-out run. ``--f0`` / ``--f1`` /
``--amp-taper-exp`` override the band's values.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate the chirp excitation target (CSV + sidecar JSON).")
    parser.add_argument(
        "--base-sidecar",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--band",
        choices=sorted(CHIRP_BANDS),
        default="low",
        help="Sweep band: low = 0.1->0.7 Hz constant amplitude; high = 0.7->3 Hz, amplitude "
             "tapered as (0.7/f)^0.5 with a 1 s fade-out (sized for kp 200/20). Sets the "
             "defaults of --f0 / --f1 / --amp-taper-exp / --ramp-up / --ramp-down.",
    )
    parser.add_argument(
        "--out-csv",
        type=str,
        default=None,
        help="Output target CSV path (default: data/chirp_<band>_target.csv).",
    )
    parser.add_argument(
        "--out-sidecar",
        type=str,
        default=None,
        help="Output sidecar JSON path (default: data/chirp_<band>_target.json).",
    )
    parser.add_argument("--duration", type=float, default=8.0, help="Trajectory duration in seconds (UR5e default; both bands).")
    parser.add_argument("--hz", type=int, default=1000, help="CSV sample rate (1000 matches the C++ collector schema).")
    # Frequency sweep (defaults from --band)
    parser.add_argument("--f0", type=float, default=None, help="Chirp start frequency [Hz] (default: band's).")
    parser.add_argument("--f1", type=float, default=None, help="Chirp end frequency [Hz] (default: band's; UR5e used 3.0 at constant amplitude).")
    parser.add_argument("--amp-taper-exp", type=float, default=None,
                        help="Amplitude taper exponent p: amplitudes scale as (f0/f)^p along the sweep "
                             "(0 = constant, 1 = constant reference velocity, 2 = constant reference "
                             "acceleration; default: band's).")
    ...
    parser.add_argument("--ramp-up", type=float, default=None, help="Linear ramp-up length [s] (default: band's, 2.0 -- UR5e).")
    parser.add_argument("--ramp-down", type=float, default=None, help="Linear ramp-down length [s] (default: band's; low 3.0 -- UR5e, high 1.0).")
...
def main() -> int:
    ...
    if args.f1 is None:
        args.f1 = band["f1"]
    if args.amp_taper_exp is None:
        args.amp_taper_exp = band["amp_taper_exp"]
    if args.ramp_up is None:
        args.ramp_up = band["ramp_up_s"]
    if args.ramp_down is None:
        args.ramp_down = band["ramp_down_s"]
    if args.out_csv is None:
        args.out_csv = f"data/chirp_{args.band}_target.csv"
    if args.out_sidecar is None:
        args.out_sidecar = f"data/chirp_{args.band}_target.json"
    ...
    payload = {
        ...
        # Recommended controller gains.  Note these are LOWER than UR5e's
    ...
    print(f"[gen_chirp_traj] band '{args.band}': {args.f0:g} -> {args.f1:g} Hz over {args.duration:g} s, "
          f"amplitude taper exponent {args.amp_taper_exp:g}, ramp {args.ramp_up:g} s up / {args.ramp_down:g} s down")
