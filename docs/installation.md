...
are for the comparison that `apply_sysid_params.py --invoke-replay` runs after
the replay (`compare_sim_real.py`; IsaacLab does not declare either). Always launch the scripts from the IsaacLab root —
the task configs reference the USD relative to it.

The installer **copies** the files, so run it again after every `git pull` of
frankatwin — otherwise IsaacLab keeps running the previous scripts and tasks.
