...
[Daemon](architecture.md#daemon-behaviour).
...
- **`multiband`** — x, y, z, yaw and roll, each driven at a low and a high
  ...
  in z and ~0.5 rad of combined yaw/roll, peaking at 0.3 m/s; it ends away from
  the start pose. `--profile heldout` is the 6-DOF variant used as the sysid
  validation run: it also tilts the tool (up to 20°), adds a third frequency up to
  2 Hz, and fades back to the start pose — roughly ±10 cm and ~0.5 rad, peaking
  at 0.45 m/s and 1.5 rad/s.
- **`chirp`** — all six axes swept from `--f0` to `--f1` (`--band low`: 0.1 → 0.7 Hz; `--band high`: 0.7 → 3 Hz with the amplitude ∝ (0.7/f)^0.5 and a 1 s fade-out, sized for kp 200/20; the low band is the fit run, the high band an optional held-out run), for
  8 s. Roughly ±10 cm in x and y, ±15 cm in z and ~0.6 rad of rotation, peaking
  at 0.46 m/s and 1.5 rad/s. This is the excitation the sysid fit uses.
...
```bash
# Go home first -- before every run: the reference is anchored at the pose the
# arm is in, and a run does not bring the joints back to where it started
python examples/move_to.py

# Default: ±5 cm along z at 0.5 Hz for 4 s
python examples/cart_impedance.py

# The sysid runs (docs/sysid.md): the low chirp band for the fit, the heldout multiband for validation,
# the high chirp band as an optional second held-out run
python examples/move_to.py
python examples/cart_impedance.py --mode chirp --band low  --kp-pos 200 --kp-ori 20 --log-1khz data/sysid/chirp_low_fit.csv
python examples/move_to.py
python examples/cart_impedance.py --mode multiband --profile heldout --kp-pos 500 --kp-ori 30 \
    --log-1khz data/sysid/multiband_heldout.csv
python examples/move_to.py
python examples/cart_impedance.py --mode chirp --band high --kp-pos 200 --kp-ori 20 --log-1khz data/sysid/chirp_high_heldout.csv
```
...
| `--log-1khz run.csv` | record every controller tick (1 kHz), setpoints stamped by the tick they took effect, and write it **on the PC** at this path, with `run_targets.csv` and the JSON sidecar `run.json` next to it. The daemon records in memory; the rows are fetched after the run and nothing is written on the NUC. Enough on its own for a sysid run; the setpoint rate stays `--rate`. A relative path resolves against the directory the script is run from ([format](data_format.md#ring-log)) |
| `--log run50.csv` | optional: the 50 Hz CSV of the PC-side state stream and its sidecar ([format](data_format.md)) |
