...
| `move_to` | `src/move_to.cpp` | One-shot joint-space (`--q`, libfranka `MotionGenerator`) or Cartesian (`--pose`, `franka::CartesianPose`, 5th-order min-jerk) motion. Used for resets. Both modes are paced by one knob, `--q-max-speed` (per-joint rad/s), and derive their motion time from the travel; for `--pose` that mapping goes through the Jacobian at the start pose, so it is an estimate. **Daemon-internal**: `examples/move_to.py` reaches it through the daemon, which stops `osc_shm`, runs this, then restarts `osc_shm` anchored at the new pose. Running it by hand skips that restart and leaves the arm with no controller; the FCI lock (`src/fci_lock.h`) refuses it outright while the daemon is up. |
