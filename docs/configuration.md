...
| `control.error_delta_pos` | **0** (pure impedance) | Per-tick position error clamp + tracking abort. `> 0` clips `\|e_pos\|∞` — bounding the push to `kp_pos · error_delta_pos` — and aborts past it; `0` disables both. The orientation channel is never clamped. Why `0` ships: [Safety chain](architecture.md#safety-chain). Runtime override via `set_gains(error_delta_pos=)` or `python examples/cart_impedance.py --err-delta-pos`. |
