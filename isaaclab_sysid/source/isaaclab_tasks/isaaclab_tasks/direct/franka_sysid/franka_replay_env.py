...
"""``Isaac-FrankaTwin-Replay-v0``: replay a recorded trajectory under the mirrored controller.

The controller is ``osc_shm``'s task-space impedance law re-implemented in
``control.py`` (``control_mode="task_impedance"``; ``"osc"`` adds the
apparent-mass projection the real robot never uses), stepped at the 1 ms
physics tick with ``decimation = 1``. Gravity is on in the scene and disabled
on the arm articulation, mirroring libfranka's gravity compensation.

``replay_python_csv_sim.py`` drives it: :meth:`set_target_pose` applies one
logged setpoint, :meth:`step_sim_no_action` advances the ticks it was held for,
and :meth:`get_state_snapshot` supplies the sim CSV row.
"""
