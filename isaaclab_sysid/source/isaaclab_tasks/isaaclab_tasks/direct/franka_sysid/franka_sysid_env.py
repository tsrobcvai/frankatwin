...
"""``Isaac-FrankaTwin-Sysid-v0``: score many actuator-parameter candidates in parallel.

Same controller, time base and gravity handling as
:mod:`franka_replay_env`, with the arm on a ``DelayedPDActuator`` so armature,
friction and motor delay can be written per env.

``sysid_franka_osc.py`` lays out one block of envs per trajectory and one env
per CMA-ES candidate, then drives them with :meth:`set_q_init_per_env`,
:meth:`set_targets`, :meth:`physics_step` and :meth:`arm_state`.
:meth:`physics_step` is the physics half of ``DirectRLEnv.step()`` alone: no
dones, rewards, resets or observations, and none of their per-tick GPU→CPU
sync.
"""
