...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Comma-separated per-trajectory weights on the loss, one per --real_csv, e.g. 1.0,1.0 for "
         "two runs (default: 1.0 for all).",
)
...
def main():
    ...
    if args.traj_weights is None:
        ...
        try:
            ...
            raise ValueError(f"--traj_weights must be comma-separated numbers, got {args.traj_weights!r}") from e
        if len(traj_w) != len(real_csvs):
            raise ValueError("--traj_weights count must match --real_csv count.")
        if any(w <= 0.0 for w in traj_w):
            raise ValueError(f"--traj_weights must all be > 0, got {traj_w}")
    ...
    print(f"traj_weights  : {', '.join(f'{w:g}' for w in traj_w)}")
    ...
    try:
        ...
        for it in range(args.max_iter):
            ...
            # What each trajectory adds to this iteration's best total: the balance to watch.
            best_weighted = (weights_t * scores)[:, best_idx].tolist()
            ...
            history.append(
                {
                    ...
                    "best_candidate_weighted": best_weighted,
                }
            )
            per_traj_str = " ".join(f"t{i}={s:.2e}" for i, s in enumerate(per_traj_scores))
            if n_traj > 1:
                per_traj_str += " | w*L of best: " + " ".join(f"t{i}={s:.2e}" for i, s in enumerate(best_weighted))
            print(
