...
Two sockets:
  - REP at tcp://*:cmd_port    -> request/reply (set_ee_target, set_gains,
                                   ...
                                   gripper_*, log_start, log_stop,
                                   log_fetch, ping, shutdown).
...
PUB path is ~1.5 KB/msg @ 100 Hz = 150 KB/s, also fine. The one exception is
`log_fetch`: its reply is two ZMQ frames, the JSON header and the rows of a
1 kHz ring log as raw little-endian float64 (a minute of it is 23.5 MB).
...
from frankatwin.ring_log import TABLES
...
WATCHDOG_POLL_S = 0.5
# Most rows one log_fetch reply carries: 20000 x 49 float64 = 7.8 MB. The REP
# loop is serial, so a bounded reply keeps every other op responsive while a
# client pulls a long log.
LOG_FETCH_MAX_ROWS = 20000
...
class FrankaTwinDaemon:
    ...
    # ---------------------------------------------------------------- dispatch
    def _dispatch(self, raw: bytes) -> List[bytes]:
        """The reply's ZMQ frames: the JSON object, then the handler's binary
        payload if it returned one (``(result, payload)`` -- only `log_fetch`)."""
        try:
        ...
        if result is None:
            result = {}
        result["ok"] = True
        frames = [json.dumps(result).encode("utf-8")]
        if payload is not None:
            frames.append(payload)
        return frames

    @staticmethod
    def _err(msg: str) -> List[bytes]:
    ...
    def _op_set_ee_target(self, req: Dict[str, Any]) -> Dict[str, Any]:
        pos = np.asarray(req["pos"], dtype=np.float64)
        quat = np.asarray(req["quat"], dtype=np.float64)
        # state_head right after the write: the tick this target takes effect
        # from (+1). The controller stamps it into a running ring log itself.
        head = self.controller.set_ee_target(pos, quat)
        return {"head": int(head)}
    ...
    # -- ring log -----------------------------------------------------------
    # 1 kHz state frames straight from the shm ring, with every set_ee_target
    # stamped by the tick it took effect (frankatwin.ring_log). Nothing is
    # written on this machine: the session is held in memory, `log_stop` merges
    # it and returns the summary, and the client pulls the rows with
    # `log_fetch` and writes the CSVs where the run was driven from. The last
    # finished log stays until the next `log_start`, so a fetch can be retried.
    # Runs under _op_lock like everything else, so start/stop never race a
    # setpoint.
    def _op_log_start(self, req: Dict[str, Any]) -> Dict[str, Any]:
        if req.get("path") is not None:
            # A client from before log_fetch, expecting the CSV on this machine.
            # Refuse now rather than record a run that nobody collects.
            raise ValueError("log_start takes no 'path' any more: the log is held in memory and "
                             "fetched by the client (log_fetch) -- update the PC checkout")
        poll_hz = req.get("poll_hz")
        return self.controller.log_start(None, poll_hz=None if poll_hz is None else float(poll_hz))

    def _op_log_stop(self, _req: Dict[str, Any]) -> Dict[str, Any]:
        return self.controller.log_stop(save=False)

    def _op_log_fetch(self, req: Dict[str, Any]) -> Tuple[Dict[str, Any], bytes]:
        """Rows [offset, offset + count) of the last finished log's `table`
        ("merged" | "targets"), at most LOG_FETCH_MAX_ROWS of them. The rows
        travel as a second frame: C-order little-endian float64, `count` x
        len(`columns`). `total` and `log_id` let the client see that every
        chunk came from the same log."""
        table = req.get("table", "merged")
        offset = int(req.get("offset", 0))
        count = req.get("count")
        count = LOG_FETCH_MAX_ROWS if count is None else min(int(count), LOG_FETCH_MAX_ROWS)
        rows = self.controller.log_fetch(table, offset, count)
        header = {
            "table": table,
            "offset": offset,
            "count": int(rows.shape[0]),
            "total": int(self.controller.log_fetch(table).shape[0]),
            "columns": TABLES[table],
            "dtype": "<f8",
            "log_id": self.controller.log_id,
        }
        return header, np.ascontiguousarray(rows, dtype="<f8").tobytes()
