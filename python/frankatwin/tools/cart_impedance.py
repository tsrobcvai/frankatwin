...
* ``multiband``: SysID v3 multi-band position excitation + optional base-yaw /
  ...
  ``frankatwin.excitation.multiband`` (the same math that produced the SysID
...
* ``chirp``: SysID v4 linear-chirp excitation (see ``frankatwin.excitation.chirp``).
...
Prerequisites:
  * On the NUC: ``frankatwin-daemon``
  * On this machine: ``pip install frankatwin`` (or ``pip install -e .``)
...
Examples:
  ...
  frankatwin-excite --mode multiband \\
  ...
  frankatwin-excite --mode multiband --amp-yaw 0.05 \\
